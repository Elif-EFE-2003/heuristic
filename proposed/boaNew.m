function [best_fitness, best_solution, curve, population_history, fitness_history] = boaNew(problem)
    ...
    % Select the best butterfly using Fitness-Distance Balance (FDB)
    best_idx = fitnessDistanceBalance(Sol, Fitness);
    fmin = Fitness(best_idx);
    best_pos = Sol(best_idx, :);
    S = Sol;
    ...
    for t = 1:N_iter
        ...
        % Update best position using FDB in each generation
        best_idx = fitnessDistanceBalance(Sol, Fitness);
        fmin = Fitness(best_idx);
        best_pos = Sol(best_idx, :);
        ...
        for i = 1:n
            ...
            if rand < p
                % Global search: move towards best butterfly (guided by FDB)
